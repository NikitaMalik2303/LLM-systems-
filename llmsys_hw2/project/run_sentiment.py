...
def binary_cross_entropy_loss(out, y):
    ...
    loss=  -(y*out.log() + (ones-y)*(ones-out).log()).mean()
    return loss
...
class Linear(minitorch.Module):
    ...
    def forward(self, x):
        ...
        g= (x@w).view(batch, self.out_size)
        return g+ self.bias.value.view(1, self.out_size)
...
class Network(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        batch, sen_len, dim= embeddings.shape
        
        x= embeddings.sum(dim=1).view(batch, dim)/ minitorch.tensor([float(sen_len)], backend=BACKEND)
        x= self.embedding(x)
        x= minitorch.dropout(x.relu(), self.dropout_prob)
        x= self.linear(x)
        x= x.sigmoid()
        return x.view(batch)
