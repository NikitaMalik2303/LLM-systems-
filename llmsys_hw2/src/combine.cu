    ...
    int fn_id)
{
  ...
  // broadcast_index(const int *big_index, const int *big_shape, const int *shape, int *out_index, int num_dims_big, int num_dims)
    ...
    const int *b_strides)
{
  ...
  for (int i=0; i<num_tiles; i++){ 
    int kA= i*TILE + threadIdx.y; 
    int kB= i*TILE + threadIdx.x; 

    if (row<m && kA<n){
      a_shared[threadIdx.x][threadIdx.y]= a_storage[batch* a_batch_stride + row*a_strides[1] + kA*a_strides[2]];
    }
    else{ 
    ...
    if (col<p && kB<n){ 
      b_shared[threadIdx.x][threadIdx.y]= b_storage[batch*b_batch_stride + kB*b_strides[1] + col*b_strides[2]];
    }
    else{ 
