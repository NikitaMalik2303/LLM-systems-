...
class CudaKernelOps(TensorOps):
    ...
    def attn_softmax_bw(out_grad: Tensor, soft_inp: Tensor):
      #   BEGIN ASSIGN4_1_2
        rows = int(np.prod(list(out_grad.shape[:-1])))
    ...
    def layernorm_fw(inp: Tensor, gamma: Tensor, beta: Tensor):
      #   BEGIN ASSIGN4_2_1
        batch_size= int(np.prod(list(inp.shape[:-1])))
        hidden_dim= int(inp.shape[-1])
        stream= torch.cuda.current_stream().cuda_stream

        out= inp.zeros(inp.shape)
        var= inp.zeros(inp.shape[:-1])
        mean= inp.zeros(inp.shape[:-1])
