...
class MultiHeadAttention(Module):
    ...
    def self_attention(self, q, kT, v):
        ...
        scores= q@kT/ tensor([np.sqrt(q_dim)], backend=self.backend) 
        if self.causal: 
        ...
        result= attn@v         # (bs, num_head, seq_len, head_dim) 
    ...
    def forward(self, x):
        ...
        out= self.out_projection(out.view(batch_size*seq_len, n_embd)).view(batch_size, seq_len, n_embd)
        return out
...
class TransformerLayer(Module):
    def __init__(self, n_embd: int, n_head: int, p_dropout: float=0.1, ln_eps: float=1e-8, bias: bool=True, backend: TensorBackend=None, use_fused_kernel: bool=False):
        ...
        if not self.use_fused_kernel:
            ...
            self.ln_1_gamma = Parameter(
                tensor_from_numpy(np.ones((n_embd,), dtype=datatype), backend=backend)
            )
            self.ln_1_beta = Parameter(
                tensor_from_numpy(np.zeros((n_embd,), dtype=datatype), backend=backend)
            )
            self.ln_2_gamma = Parameter(
                tensor_from_numpy(np.ones((n_embd,), dtype=datatype), backend=backend)
            )
            self.ln_2_beta = Parameter(
                tensor_from_numpy(np.zeros((n_embd,), dtype=datatype), backend=backend)
            )
    ...
    def forward(self, x):
        ...
        if not self.use_fused_kernel:
            # COPY FROM ASSIGN2_4
            x_1 = self.ln_1(x.view(batch_size*seq_len, x_dim)).view(batch_size, seq_len, x_dim)
            x_1= self.attention(x_1)
            x+= x_1

            x_2= self.ln_2(x.view(batch_size*seq_len, x_dim)).view(batch_size, seq_len, x_dim)
            x_2= self.ff(x_2)
            x+= x_2

        else:
            # BEGIN ASSIGN3_3
            x_1 = x.view(batch_size * seq_len, x_dim).layernorm(
                self.ln_1_gamma.value, self.ln_1_beta.value
            ).view(batch_size, seq_len, x_dim)
            x = x + self.attention(x_1)

            x_2 = x.view(batch_size * seq_len, x_dim).layernorm(
...
class DecoderLM(Module):
    def __init__(
        ...
        if not self.use_fused_kernel:
            ...
            # BEGIN ASSIGN3_3
            self.ln_gamma = Parameter(
                tensor_from_numpy(np.ones((n_embd,), dtype=datatype), backend=backend)
            )
            self.ln_beta = Parameter(
                tensor_from_numpy(np.zeros((n_embd,), dtype=datatype), backend=backend)
            )
            # END ASSIGN3_3
    ...
    def forward(self, idx):
        ...
        pos= self.position_embeddings(tensor_from_numpy(np.arange(seq_len, dtype=np.int32).reshape(1, seq_len), backend=idx.backend))
