...
__global__ void ker_layer_norm(T *ln_res, T *vars, T *means, const T *inp,
  ...
  __shared__ float s_mean, s_var, s_rstd; 

  if (threadIdx.x==0){
    s_mean= l_sum[0]/(hidden_size*4);
    s_var= l_sum2[0]/(hidden_size*4)-s_mean*s_mean;
    s_rstd= rsqrtf(s_var+LN_EPSILON);
  ...
  for (uint i=threadIdx.x; i<hidden_size; i+=blockDim.x){
    float4 val= inp_f4[i];
    float4 gamma= scale_f4[i];
    float4 beta= bias_f4[i];
    val.x= (val.x-s_mean)*s_rstd*gamma.x + beta.x;
    val.y= (val.y-s_mean)*s_rstd*gamma.y + beta.y;
    val.z= (val.z-s_mean)*s_rstd*gamma.z + beta.z;
    val.w= (val.w-s_mean)*s_rstd*gamma.w + beta.w;
    out_f4[i]= val;
  }
  /// END ASSIGN4_2_1
}
...
__global__ void ker_ln_bw_dgamma_dbetta(T *gamma_grad, T *betta_grad,
  ...
  // Step 1
  const int col= blockIdx.x * TILE_DIM + threadIdx.y;

  float l_dgamma= 0, l_dbeta= 0;
  if (col<width){
    for (int i=threadIdx.x; i<rows; i+=TILE_DIM){
      int idx= i*width + col;
      float dy= (float)out_grad[idx]; 
      float mean= (float)means[i];
      float var= (float)vars[i];
      float x_hat= ((float)inp[idx]-mean)/sqrtf(var);
      l_dgamma+= dy*x_hat;
      l_dbeta+= dy;
    }
  }
  ...
  // Step 2
  betta_buffer[threadIdx.y][threadIdx.x]= l_dbeta;
  gamma_buffer[threadIdx.y][threadIdx.x]= l_dgamma;
  __syncthreads();
  
  // Step 3
  float db= betta_buffer[threadIdx.y][threadIdx.x];
  float dg= gamma_buffer[threadIdx.y][threadIdx.x];
...
__global__ void ker_ln_bw_dinp(T *inp_grad, const T *out_grad, const T *inp,
  ...
  // Step 1
  const float4 *dy_f4= reinterpret_cast<const float4 *>(out_grad) + blockIdx.x * hidden_dim;
  const float4 *x_f4= reinterpret_cast<const float4 *>(inp) + blockIdx.x * hidden_dim;
  ...
  l_sum1[0]= 0.f;
  l_sum2[0]= 0.f;
  const float mean= (float)means[blockIdx.x];
  const float var= (float)vars[blockIdx.x];
  const float rstd= rsqrtf(var);
  for (int i=threadIdx.x; i<hidden_dim; i+=blockDim.x){

    float dxhat0, dxhat1, dxhat2, dxhat3;
    float xhat0, xhat1, xhat2, xhat3;
    float4 dy, x, g;
    ...
    l_sum1[0]+= dxhat0 + dxhat1 + dxhat2 + dxhat3;
    l_sum2[0]+= dxhat0*xhat0 + dxhat1*xhat1 + dxhat2*xhat2 + dxhat3*xhat3;
  }
                                
  // Step 2
   
  // Step 3
  blockReduce<ReduceType::kSum, 1>(l_sum1);
  blockReduce<ReduceType::kSum, 1>(l_sum2);
  ...
  for (int i=threadIdx.x; i<hidden_dim; i+=blockDim.x){
    ...
    float4 out; 
    out.x= (dxhat0 - (s_sum1 + xhat0*s_sum2)/(hidden_dim*4))*rstd;
    out.y= (dxhat1 - (s_sum1 + xhat1*s_sum2)/(hidden_dim*4))*rstd;
    out.z= (dxhat2 - (s_sum1 + xhat2*s_sum2)/(hidden_dim*4))*rstd;
    out.w= (dxhat3 - (s_sum1 + xhat3*s_sum2)/(hidden_dim*4))*rstd;
