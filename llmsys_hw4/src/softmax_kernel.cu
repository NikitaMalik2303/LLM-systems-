...
__global__ void ker_attn_softmax(T *inp, const T *attn_mask, int from_len,
  ...
  for (int token_id = blockIdx.x * token_per_reduce; token_id < from_len;
    ...
    for (int i=0; i<token_per_reduce; i++){
      ...
      for (int j=0; j<ele_per_thread; j++){
        float temp_val; 
        int col_id= ele_per_thread*threadIdx.x + j;
        if (mask_future && col_id > token_id + i){
          temp_val = REDUCE_FLOAT_INF_NEG;
        }else{
          temp_val = (float)inp_val[i][j];
          if (attn_mask){
...
void launch_attn_softmax_bw(float *out_grad,
  ...
  cudaMalloc((void **)&d_soft_outp, rows * softmax_len * sizeof(float));

  cudaMemcpy(d_out_grad, out_grad, rows * softmax_len * sizeof(float), cudaMemcpyHostToDevice);
  cudaMemcpy(d_soft_outp, soft_outp, rows * softmax_len * sizeof(float), cudaMemcpyHostToDevice);
  // Launch kernel
